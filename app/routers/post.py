...
def get_posts( db: Session = Depends(get_db),current_user:int = Depends(
   ...
   #posts = db.query(models.Post).filter(
    ...
    posts = (db.query(models.Post, func.count(models.Vote.post_id).label(
        'likes')).join(models.Vote, models.Vote.post_id == models.Post.id, isouter=True).group_by
    (models.Post.id).filter(
        models.Post.title.contains(search)).limit(Limit).offset(skip).all())
   
  
    return posts
     # to get a specific users post
...
def create_posts(post: schemas.PostCreate, db: Session = Depends(get_db), current_user:int = Depends(
 ...
 #   cursor.execute(""" INSERT INTO public.posts (title, content, published) VALUES (%s, %s, %s)
  #                RETURNING *""",
...
def delete_post(id:int,  db: Session = Depends(get_db), current_user:int = Depends(
   ...
   # cursor.execute("""DELETE FROM public.posts  WHERE id = %s returning *""", (str(id),))
    ...
    if post.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not  authorized to perform such requested action")
